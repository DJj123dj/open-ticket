...
export const registerCommandResponders = async () => {
    ...
    opendiscord.responders.commands.add(new api.ODCommandResponder("openticket:close",generalConfig.data.prefix,"close"))
    opendiscord.responders.commands.get("openticket:close").workers.add([
        ...
        new api.ODWorker("openticket:close",0,async (instance,params,source,cancel) => {
            ...
            await opendiscord.actions.get("openticket:close-ticket").run(source,{guild,channel,user,ticket,reason,sendMessage:false})
            await instance.reply(await opendiscord.builders.messages.getSafe("openticket:close-message").build(source,{guild,channel,user,ticket,reason}))
        }),
...
export const registerButtonResponders = async () => {
    ...
    opendiscord.responders.buttons.add(new api.ODButtonResponder("openticket:close-ticket",/^od:close-ticket_/))
    opendiscord.responders.buttons.get("openticket:close-ticket").workers.add(
        new api.ODWorker("openticket:close-ticket",0,async (instance,params,source,cancel) => {
            ...
            if (originalSource == "ticket-message") await opendiscord.verifybars.get("openticket:close-ticket-ticket-message").activate(instance)
            else await opendiscord.verifybars.get("openticket:close-ticket-reopen-message").activate(instance)
        })
    )
}
...
export const registerModalResponders = async () => {
    ...
    opendiscord.responders.modals.get("openticket:close-ticket-reason").workers.add([
        new api.ODWorker("openticket:close-ticket-reason",0,async (instance,params,source,cancel) => {
            ...
            if (originalSource == "ticket-message"){
                ...
                await opendiscord.actions.get("openticket:close-ticket").run(originalSource,{guild,channel,user,ticket,reason,sendMessage:true})
                ...
                await instance.defer("update",false)
                await opendiscord.actions.get("openticket:close-ticket").run(originalSource,{guild,channel,user,ticket,reason,sendMessage:false})
