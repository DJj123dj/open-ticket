...
export const registerCommandResponders = async () => {
    ...
    opendiscord.responders.commands.add(new api.ODCommandResponder("openticket:remove",generalConfig.data.prefix,"remove"))
    opendiscord.responders.commands.get("openticket:remove").workers.add([
        ...
        new api.ODWorker("openticket:remove",0,async (instance,params,source,cancel) => {
            ...
            if (!participants || !participants.find((p) => p.user.id == data.id && p.role == "participant")){
                instance.reply(await opendiscord.builders.messages.getSafe("openticket:error").build("button",{guild,channel,user,error:opendiscord.languages.getTranslation("errors.actionInvalid.remove"),layout:"simple"}))
                return cancel()
            }

            //start removing user from ticket
            await instance.defer(false)
            await opendiscord.actions.get("openticket:remove-ticket-user").run(source,{guild,channel,user,ticket,reason,sendMessage:false,data})
            await instance.reply(await opendiscord.builders.messages.getSafe("openticket:remove-message").build(source,{guild,channel,user,ticket,reason,data}))
        }),
