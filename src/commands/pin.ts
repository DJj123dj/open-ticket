...
export const registerCommandResponders = async () => {
    ...
    opendiscord.responders.commands.add(new api.ODCommandResponder("openticket:pin",generalConfig.data.prefix,"pin"))
    opendiscord.responders.commands.get("openticket:pin").workers.add([
        ...
        new api.ODWorker("openticket:pin",0,async (instance,params,source,cancel) => {
            ...
            await opendiscord.actions.get("openticket:pin-ticket").run(source,{guild,channel,user,ticket,reason,sendMessage:false})
            await instance.reply(await opendiscord.builders.messages.getSafe("openticket:pin-message").build(source,{guild,channel,user,ticket,reason}))
        }),
...
export const registerButtonResponders = async () => {
    ...
    opendiscord.responders.buttons.add(new api.ODButtonResponder("openticket:pin-ticket",/^od:pin-ticket/))
    opendiscord.responders.buttons.get("openticket:pin-ticket").workers.add(
        new api.ODWorker("openticket:pin-ticket",0,async (instance,params,source,cancel) => {
            ...
            if (originalSource == "ticket-message") await opendiscord.verifybars.get("openticket:pin-ticket-ticket-message").activate(instance)
            else await opendiscord.verifybars.get("openticket:pin-ticket-unpin-message").activate(instance)
        })
    )
}
...
export const registerModalResponders = async () => {
    ...
    opendiscord.responders.modals.get("openticket:pin-ticket-reason").workers.add([
        new api.ODWorker("openticket:pin-ticket-reason",0,async (instance,params,source,cancel) => {
            ...
            if (originalSource == "ticket-message"){
                ...
                await opendiscord.actions.get("openticket:pin-ticket").run(originalSource,{guild,channel,user,ticket,reason,sendMessage:true})
                ...
                await instance.defer("update",false)
                await opendiscord.actions.get("openticket:pin-ticket").run(originalSource,{guild,channel,user,ticket,reason,sendMessage:false})
