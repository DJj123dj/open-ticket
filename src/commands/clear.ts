...
export const registerCommandResponders = async () => {
    ...
    opendiscord.responders.commands.add(new api.ODCommandResponder("openticket:clear",generalConfig.data.prefix,"clear"))
    opendiscord.responders.commands.get("openticket:clear").workers.add([
        ...
        new api.ODWorker("openticket:clear",0,async (instance,params,source,cancel) => {
            ...
            await instance.reply(await opendiscord.builders.messages.getSafe("openticket:clear-verify-message").build(source,{guild,channel,user,filter,list}))
        }),
...
export const registerButtonResponders = async () => {
    ...
    opendiscord.responders.buttons.add(new api.ODButtonResponder("openticket:clear-continue",/^od:clear-continue_/))
    opendiscord.responders.buttons.get("openticket:clear-continue").workers.add(
        new api.ODWorker("openticket:clear-continue",0,async (instance,params,source,cancel) => {
            ...
            await opendiscord.actions.get("openticket:clear-tickets").run(originalSource,{guild,channel,user,filter,list:ticketList})
            await instance.update(await opendiscord.builders.messages.getSafe("openticket:clear-message").build(originalSource,{guild,channel,user,filter,list}))
        })
    )
}