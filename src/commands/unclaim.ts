...
export const registerCommandResponders = async () => {
    ...
    opendiscord.responders.commands.add(new api.ODCommandResponder("openticket:unclaim",generalConfig.data.prefix,"unclaim"))
    opendiscord.responders.commands.get("openticket:unclaim").workers.add([
        ...
        new api.ODWorker("openticket:unclaim",0,async (instance,params,source,cancel) => {
            ...
            await opendiscord.actions.get("openticket:unclaim-ticket").run(source,{guild,channel,user,ticket,reason,sendMessage:false})
            await instance.reply(await opendiscord.builders.messages.getSafe("openticket:unclaim-message").build(source,{guild,channel,user,ticket,reason}))
        }),
...
export const registerButtonResponders = async () => {
    ...
    opendiscord.responders.buttons.add(new api.ODButtonResponder("openticket:unclaim-ticket",/^od:unclaim-ticket/))
    opendiscord.responders.buttons.get("openticket:unclaim-ticket").workers.add(
        new api.ODWorker("openticket:unclaim-ticket",0,async (instance,params,source,cancel) => {
            ...
            if (originalSource == "ticket-message") await opendiscord.verifybars.get("openticket:unclaim-ticket-ticket-message").activate(instance)
            else await opendiscord.verifybars.get("openticket:unclaim-ticket-claim-message").activate(instance)
        })
    )
}
...
export const registerModalResponders = async () => {
    ...
    opendiscord.responders.modals.get("openticket:unclaim-ticket-reason").workers.add([
        new api.ODWorker("openticket:unclaim-ticket-reason",0,async (instance,params,source,cancel) => {
            ...
            if (originalSource == "ticket-message"){
                ...
                await opendiscord.actions.get("openticket:unclaim-ticket").run(originalSource,{guild,channel,user,ticket,reason,sendMessage:true})
                ...
                await instance.defer("update",false)
                await opendiscord.actions.get("openticket:unclaim-ticket").run(originalSource,{guild,channel,user,ticket,reason,sendMessage:false})
