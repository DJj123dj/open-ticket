...
export const registerCommandResponders = async () => {
    ...
    opendiscord.responders.commands.add(new api.ODCommandResponder("openticket:rename",generalConfig.data.prefix,"rename"))
    opendiscord.responders.commands.get("openticket:rename").workers.add([
        ...
        new api.ODWorker("openticket:rename",0,async (instance,params,source,cancel) => {
            ...
            await opendiscord.actions.get("openticket:rename-ticket").run(source,{guild,channel,user,ticket,reason,sendMessage:false,data:name})
            await instance.reply(await opendiscord.builders.messages.getSafe("openticket:rename-message").build(source,{guild,channel,user,ticket,reason,data:name}))
        }),
